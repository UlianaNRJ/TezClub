...
<div class="openid-block step-two">
	<h1>{$aLang.openid_continue}</h1>

	<ul>
		<li {if !$_aRequest.submit_mail}class="active"{/if} id="li_data"><a href="javascript:showFormData()" ><span>{$aLang.openid_type_new}</span></a></li>
		<li id="li_mail" {if $_aRequest.submit_mail}class="active"{/if}><a href="javascript:showFormMail()" ><span>{$aLang.openid_type_exists}</span></a></li>
	</ul>
	
	
	<form method="post" action="{router page='login'}openid/data/" id="form_data" {if $_aRequest.submit_mail}style="display: none;"{/if}>			
		<p>
			<label>{$aLang.openid_login}</label>
		...
		<p style="margin-bottom: 18px;">
			{if $oConfig->GetValue('plugin.openid.mail_required')}
				<label>{$aLang.openid_mail}</label>
			{else}
				<a href="javascript:toggleMail()" class="openid-mail">{$aLang.openid_mail_toggle}</a>
			{/if}
		...
		<a href="#" class="openid-ok" name="submit_data"  onclick="getEl('form_data').submit(); return false;"><span>{$aLang.openid_type_new_send}</span></a>		
	</form>
	...
	<form method="post" action="{router page='login'}openid/data/"  id="form_mail" {if $_aRequest.submit_mail}style="display: block;"{else}style="display: none;"{/if}>							
		...
		<a href="#" class="openid-ok"  onclick="getEl('form_mail').submit(); return false;"><span>{$aLang.openid_type_exists_send}</span></a>			
	</form>
</div>	

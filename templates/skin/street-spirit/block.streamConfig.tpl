...
<div class="block stream-settings">
	<div class="block-header-conteiner"><h2><span>{$aLang.stream_block_config_title}</span></h2></div>
...
<ul class="stream-settings-filter">
	...
	<div class="stream-settings-userlist">
		<p><input type="text" id="stream_users_complete"/>
						...
						{foreach from=$aStreamSubscribedUsers item=oUser}
							...
							{if !isset($aStreamFriends.$iUserId)}
								<li><input class="streamUserCheckbox input-checkbox"
											...
											id="strm_u_{$iUserId}"
											checked="checked"
											onClick="if ($(this).prop('checked')) { ls.stream.subscribe({$iUserId}) } else { ls.stream.unsubscribe({$iUserId}) } " />

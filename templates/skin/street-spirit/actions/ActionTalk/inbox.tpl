...
<form action="{router page='talk'}" method="post" id="form_talks_list">
	...
	<table class="table table-people table-talk">
			...
			<tr class="table">
				<td class="width20"><input type="checkbox" name="" onclick="ls.tools.checkAll('form_talks_checkbox', this);"></td>
				<td class="width150">{$aLang.talk_inbox_target}</td>
				<td class="width20"></td>
				<td>{$aLang.talk_inbox_title}</td>
				<td class="width170" align="center">{$aLang.talk_inbox_date}</td>
			</tr>
	...
	<input type="submit" name="submit_talk_del" class="button" value="{$aLang.talk_inbox_delete}" onclick="return ($('.form_talks_checkbox:checked').length==0)?false:confirm('{$aLang.talk_inbox_delete_confirm}');" />
</form>

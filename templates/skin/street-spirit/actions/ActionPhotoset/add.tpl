...
if (jQuery.browser.flash) {
	ls.photoset.initSwfUpload({
		post_params: { 'topic_id':{json var=$_aRequest.topic_id|default:''} }
	});
}
</script>
...
<div class="topic" style="display: none;">
	...
	<form action="" method="post" enctype="multipart/form-data">
		...
		<textarea name="topic_text" id="topic_text" cols="20" rows="20" class="input-wide">{$_aRequest.topic_text}</textarea>
		</p>
                                ...
                                {if count($aPhotos)}
                                    {foreach from=$aPhotos item=oPhoto}
                                        ...
                                        <li id="photo_{$oPhoto->getId()}" {if $bIsMainPhoto}class="marked-as-preview"{/if}>
                                            ...
                                            <textarea onBlur="ls.photoset.setPreviewDescription({$oPhoto->getId()}, this.value)" cols="20" rows="10">{$oPhoto->getDescription()}</textarea><br />

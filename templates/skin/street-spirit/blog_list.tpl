				...
				{if $oUserCurrent}
					<td align="center">
						{if $oUserCurrent->getId()!=$oBlog->getOwnerId() and $oBlog->getType()=='open'}
							<div onclick="ls.blog.toggleJoin(this, {$oBlog->getId()}); return false;" class="join {if $oBlog->getUserIsJoin()}active{/if}"><span>{if $oBlog->getUserIsJoin()}покинуть обсуждение{else}{$aLang.blog_join}{/if}</span></div>
						{else}
							&mdash;
						{/if}
					</td>
